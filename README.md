...
국회 공공데이터 포털(Java 8 / Spring MVC 4 / iBatis, 비공개 소스)을 운영하며 설계했던 **동적 API 생성 엔진**을 현대 스택으로 재설계한 프로젝트입니다. 레거시에서 경험한 구조적 문제를 설계 차원에서 해결하는 것이 목표입니다.
...
| 수동 테스트 | 단위·통합 테스트 26건 + GitHub Actions CI |

## 아키텍처

```
관리자                          사용자
  │ POST /admin/api/datasets      │ GET /api/v1/datasets/bills
  │ POST /{key}/publish           │     ?COMMITTEE=행정안전위원회
  ▼                               │     &sort=PROPOSE_DT,desc&format=csv
┌─────────────────┐               ▼
│ DatasetAdmin    │        ┌──────────────────┐
...
│  - 발행 전       │                 ▼
│    실존 프로브   │        ┌──────────────────┐    ┌──────────────────┐
└───────┬─────────┘        │ DataQueryService  │───▶│ DynamicQuery     │
        ▼                  └────────┬─────────┘    │ Builder (jOOQ)   │
┌─────────────────┐                 │              │  - 칼럼 화이트리스트│
│ DATASET /       │◀── 메타데이터 로드│              │  - 값 바인드 처리  │
│ DATASET_COLUMN  │                 ▼              └──────────────────┘
└─────────────────┘        ┌──────────────────┐
                           │ ResponseWriter    │  json / csv / xml
                           │ Resolver (전략)   │  (+구현체 추가로 확장)
                           └──────────────────┘
```
...
시드 데이터(국회 의안 정보, 가상 데이터 15건)가 자동 등록·발행됩니다.
